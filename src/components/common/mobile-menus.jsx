    ...
    <>
      <nav className="tp-main-menu-content">
        {mobile_menu.map((menu, i) => (
          <ul key={i}>
            {menu.homes ? (
              <li className={`has-dropdown has-mega-menu ${isActiveMenu === menu.name ? 'dropdown-opened':''}`}>
                <a className={`${isActiveMenu === menu.name ? 'expanded':''}`}>
                  Home
                  <button onClick={()=> handleOpenSubMenu(menu.name)} className={`dropdown-toggle-btn ${isActiveMenu === menu.name ? 'dropdown-opened':''}`}>
                    <i className="fa-regular fa-angle-right"></i>
                  </button>
                </a>
                <div className={`home-menu tp-submenu tp-mega-menu ${isActiveMenu === menu.name ? 'active':''}`}>
          ...
          <div className="row row-cols-lg-4 row-cols-sm-2 row-cols-1 gx-2 gy-2 gy-lg-0">
            {instagram_data.map((item) => (
                        ...
                        <div className="home-menu-item">
                          {/* <Link href={home.link}>
                            ...
                            <div className="home-menu-content">
                              <h5 className="home-menu-title">{home.name}</h5>
                            </div>
                          </Link> */}
                        </div>
                      </div>
              ...
              <li key={menu.id} className={`has-dropdown ${isActiveMenu === menu.name ? 'dropdown-opened':''}`}>
                <a className={`${isActiveMenu === menu.name ? 'expanded':''}`}>
                  {menu.name}
                  <button onClick={()=> handleOpenSubMenu(menu.name)} className={`dropdown-toggle-btn ${isActiveMenu === menu.name ? 'dropdown-opened':''}`}>
                    <i className="fa-regular fa-angle-right"></i>
                ...
                <ul className={`tp-submenu ${isActiveMenu === menu.name ? 'active':''}`}>
                  {menu.sub_menus.map((b, i) => (
                    <li key={i}>
                      <Link href={b.link}>{b.name}</Link>
                    </li>
                  ))}
                </ul>
              </li>
